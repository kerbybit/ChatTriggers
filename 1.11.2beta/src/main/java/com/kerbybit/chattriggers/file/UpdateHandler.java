            ...
            try {
                ...
                for (String getline : lines) {
                    ...
                    if (getline.contains(getthis)) {
                        ...
                        try {
                            if (getuuid.equals(Minecraft.getMinecraft().player.getUniqueID().toString().replace("-", ""))) {
                                global.canUse = false;
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                ...
                web = new URL(global.hasWatermarkURL+"?send="+Minecraft.getMinecraft().player.getUniqueID());
                fis = web.openStream();
                lines = new ArrayList<>();
                bufferedReader = new BufferedReader(new InputStreamReader(fis,"UTF-8"));
                while ((line = bufferedReader.readLine()) != null) {
                ...
                String myuuid = Minecraft.getMinecraft().player.getUniqueID().toString().replace("-","");
