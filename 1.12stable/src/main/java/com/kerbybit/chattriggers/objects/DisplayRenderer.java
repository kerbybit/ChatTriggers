...
public class DisplayRenderer {
    ...
    public static void drawDisplays(RenderGameOverlayEvent event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.TEXT) {
            ...
            FontRenderer ren = Minecraft.getMinecraft().fontRenderer;
            ...
            if (display_text.contains("<rainbow>") || (display_text.contains("<rainbow=") && display_text.contains(">"))) {
                ...
                if (display_text.contains("<rainbow=") && display_text.contains(">")) {
                    ...
                    try {
                        ...
                        if (global.debug) {
                            ChatHandler.warn("gray", "<rainbow=&n> - &n must be a number!");
                        }
                    }
    ...
    public static void drawRect(double left, double top, double right, double bottom, int color)
    {
        ...
        bufferbuilder.pos(left+1, bottom-1, 0.0D).endVertex();
        bufferbuilder.pos(right-2, bottom-1, 0.0D).endVertex();
        bufferbuilder.pos(right-2, top-1, 0.0D).endVertex();
        bufferbuilder.pos(left+1, top-1, 0.0D).endVertex();
        tessellator.draw();
