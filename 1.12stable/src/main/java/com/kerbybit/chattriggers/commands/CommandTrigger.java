...
public class CommandTrigger extends CommandBase {
    ...
    private static void commandSimulate(String args[]) {
        ...
        ClientChatReceivedEvent chatEvent = new ClientChatReceivedEvent(ChatType.CHAT, ITextComponent.Serializer.jsonToComponent("{text:'"+TMP_e.toString().replace("'", "\\'").trim()+"'}"));
            ...
            if (args[1].equalsIgnoreCase("DEBUG")) {
                ...
                if (args.length == 2) {
                    for (String fmsg : global.chatHistory) {
                        ...
                        tmp_out = tmp_out.replace("'", "\\'").replace("\"", "\\");
