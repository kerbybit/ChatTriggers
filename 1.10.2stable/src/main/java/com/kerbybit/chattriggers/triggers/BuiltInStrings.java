...
public class BuiltInStrings {
    ...
    public static String builtInStrings(String TMP_e, ClientChatReceivedEvent chatEvent, Boolean isAsync) {
        ...
        if (TMP_e.contains("{yaw}")) {
            TMP_e = createDefaultString("yaw", String.valueOf(MathHelper.wrapDegrees(mc.thePlayer.rotationYaw)), TMP_e, isAsync);
        }
        if (TMP_e.contains("{pitch}")) {
            TMP_e = createDefaultString("pitch", String.valueOf(MathHelper.wrapDegrees(mc.thePlayer.rotationPitch)), TMP_e, isAsync);
        }
        ...
        if (TMP_e.contains("{lightLevel}")) {
        	TMP_e = createDefaultString("lightLevel", mc.theWorld.getLight(mc.thePlayer.getPosition()) + "", TMP_e, isAsync);
		}
        ...
        if (TMP_e.contains("{heldItemOffhand}")) {
            ItemStack item = Minecraft.getMinecraft().thePlayer.getHeldItem(EnumHand.OFF_HAND);
		...
		if (TMP_e.contains("{biome}")) {
        	...
        	Biome biome = chunk.getBiome(mc.thePlayer.getPosition(),
					mc.theWorld.getBiomeProvider());
