...
public class TriggerHandler {
	...
	public static void onRightClickPlayer(PlayerInteractEvent e) {
		for (int i=0; i<global.onRightClickPlayerTrigger.size(); i++) {
			//add all events to temp list
			List<String> TMP_events = new ArrayList<String>();
			for (int j=2; j<global.onRightClickPlayerTrigger.get(i).size(); j++) {TMP_events.add(global.onRightClickPlayerTrigger.get(i).get(j).replace("{player}", e.getEntity().getName()));}
